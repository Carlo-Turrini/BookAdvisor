...
export class RecensioneService {
  ...
  public deleteReview(userId: number, reviewId: number): Promise<void> {
    return this.http.delete<void>('http://localhost:8080/api/utenti/' + userId + '/recensioni/' + reviewId, {withCredentials: true}).toPromise();
  }

  public allReviewsByUser(userId: number): Promise<Recensione[]> {
    return this.http.get<Recensione[]>('http://localhost:8080/api/utenti/' + userId + '/recensioni', {withCredentials: true}).toPromise();
  }
  ...
  public addUsefulReview(reviewID: number, userID: number): Promise<void> {
    return this.http.post<void>("http://localhost:8080/api/recensioni/" + reviewID + "/isReviewUseful", userID, {withCredentials:true} ).toPromise();
  }

  public removeUsefulReview(reviewID: number, userID: number): Promise<void> {
    return this.http.delete<void>("http://localhost:8080/api/recensioni/" + reviewID + "/isReviewUseful/" + userID, {withCredentials: true}).toPromise();
  }
}
