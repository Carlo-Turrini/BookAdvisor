...
public interface BookRankingRepository extends JpaRepository<BookRanking, Integer> {
    ...
    @Query("SELECT new com.student.book_advisor.data_persistency.model.dto.BookRankingDTO(br.id, br.bookRank, b.id, b.titolo) FROM BookRanking br JOIN MyBooks mb ON (br.myBooks.id = mb.id) JOIN mb.book b WHERE mb.usersInfo.id = :userID ORDER BY br.bookRank ASC")
    ...
    @Query("SELECT new com.student.book_advisor.data_persistency.model.dto.auxiliaryDTOs.BookRankRemovalInfoDTO(mb.usersInfo.id, br.id) FROM BookRanking br JOIN MyBooks mb ON (br.myBooks.id = mb.id) WHERE mb.book.id = :bookID")
