...
public interface PrizeRepository extends JpaRepository<Prize, Integer> {
    ...
    @Query("SELECT new com.student.book_advisor.data_persistency.model.dto.PrizeDTO(p.id, p.yearAwarded, p.prizeName) FROM Prize p WHERE p.book.id = :bookID")
    ...
    @Query("SELECT new com.student.book_advisor.data_persistency.model.dto.PrizeDTO(p.id, p.yearAwarded, p.prizeName) FROM Prize p WHERE p.book.id = :bookID AND p.prizeName = :prizeName")
