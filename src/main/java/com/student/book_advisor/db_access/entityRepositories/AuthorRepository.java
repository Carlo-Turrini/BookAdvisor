...
public interface AuthorRepository extends JpaRepository<Author, Integer> {
    ...
    @Query("SELECT new com.student.book_advisor.db_access.dto.AuthorCardDTO(a.id, a.authorsFullname, a.birthYear, a.deathYear) FROM Author a")
    public List<AuthorCardDTO> findAllToDTO();

    @Query("SELECT new com.student.book_advisor.db_access.dto.AuthorDTO(a.id, a.authorsFullname, a.biography, a.birthYear, a.deathYear) FROM Author a WHERE a.id = :id")
    public AuthorDTO getAuthorsDTOById(@Param("id")Integer id);

    @Query("SELECT new com.student.book_advisor.db_access.dto.auxiliaryDTOs.AuthorOfBook(a.id, a.authorsFullname) FROM Author a JOIN a.authorJoinBookList authBook WHERE authBook.book.id = :bookID")
    ...
    @Query("SELECT new com.student.book_advisor.db_access.dto.auxiliaryDTOs.AuthorOfBook(a.id, a.authorsFullname) FROM Author a")
    public List<AuthorOfBook> findAllAuthorsOfBook();

    @Query("SELECT new com.student.book_advisor.db_access.dto.auxiliaryDTOs.AuthorOfBook(a.id, a.authorsFullname) FROM Author a WHERE a.authorsFullname = :fullname")
