...
public interface LibroRepository extends JpaRepository<Libro, Integer> {
    @Query("SELECT DISTINCT new com.student.book_advisor.db_access.dto.LibroCardDTO(l.id, l.titolo, AVG(r.rating) ) FROM Libro l JOIN l.genreJoinBookList genres LEFT JOIN Recensione r ON (r.libro.id = l.id)  WHERE genres.genre.genre = :genere GROUP BY l.id, l.titolo ORDER BY AVG(r.rating) DESC ")
    public List<LibroCardDTO> findLibriByGenere(@Param("genere")String genere);

    @Query("SELECT DISTINCT new com.student.book_advisor.db_access.dto.LibroCardDTO(l.id, l.titolo, AVG(r.rating)) FROM Libro l LEFT JOIN Recensione r ON (r.libro.id = l.id) GROUP BY l.id, l.titolo ORDER BY AVG(r.rating) DESC ")
    ...
    @Query("SELECT new com.student.book_advisor.db_access.dto.LibroCardDTO(l.id, l.titolo, AVG(r.rating)) FROM Libro l JOIN l.saga s LEFT JOIN Recensione r ON (r.libro.id = l.id) WHERE s.sagaTitle = :titoloSaga AND l.id <> :bookId GROUP BY l.id, l.titolo")
    ...
    @Query("SELECT new com.student.book_advisor.db_access.dto.LibroCardDTO(l.id, l.titolo, AVG(r.rating)) FROM Libro l JOIN l.authorJoinBookList  ajbl JOIN ajbl.author a LEFT JOIN Recensione r ON (r.libro.id = l.id) WHERE a.authorsFullname = :authorsFullname GROUP BY l.id, l.titolo")
