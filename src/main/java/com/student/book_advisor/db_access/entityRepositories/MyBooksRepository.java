...
public interface MyBooksRepository extends JpaRepository<MyBooks, Integer> {
    ...
    @Query("SELECT new com.student.book_advisor.db_access.dto.auxiliaryDTOs.MyBooksReadDTO(mb.id, b.titolo) FROM MyBooks mb JOIN Libro b ON (mb.book.id = b.id) WHERE mb.ShelfType = 'read' AND mb.id NOT IN (SELECT br.myBooks.id FROM BookRanking br) ")
