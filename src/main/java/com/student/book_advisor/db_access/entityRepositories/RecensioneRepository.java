...
public interface RecensioneRepository extends JpaRepository<Recensione, Integer> {
    ...
    @Query("SELECT new com.student.book_advisor.db_access.dto.auxiliaryDTOs.OverallRatingsForBook(AVG(r.rating), AVG(r.originalityRating), AVG(r.pageTurnerRating), AVG(r.writingQualityRating)) FROM Recensione r WHERE r.libro.id = :bookID")
