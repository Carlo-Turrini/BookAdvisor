...
import com.student.book_advisor.db_access.dto.UsersInfoDTO;
import com.student.book_advisor.db_access.dto.UtenteCardDTO;
...
public interface UsersInfoRepository extends JpaRepository<UsersInfo, Integer> {
    ...
    @Query("SELECT new com.student.book_advisor.db_access.dto.UsersInfoDTO(ui.id, ui.name, ui.surname, ui.username, ui.email, ui.description) FROM UsersInfo ui WHERE ui.id = :id")
    public UsersInfoDTO findUserById(@Param("id")Integer id);

    @Query("SELECT new com.student.book_advisor.db_access.dto.UtenteCardDTO(ui.id, ui.username, ui.name, ui.surname) FROM UsersInfo ui")
