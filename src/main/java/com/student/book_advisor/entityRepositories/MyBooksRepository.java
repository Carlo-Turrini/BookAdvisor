...
public interface MyBooksRepository extends JpaRepository<MyBooks, Long> {
    ...
    @Query("SELECT new com.student.book_advisor.dto.auxiliaryDTOs.MyBooksReadDTO(mb.id, b.titolo) FROM MyBooks mb JOIN Libro b ON (mb.book.id = b.id) WHERE mb.ShelfType = 'read' AND mb.id NOT IN (SELECT br.myBooks.id FROM BookRanking br) ")
    public List<MyBooksReadDTO> getAllMyBooksReadButNotInRank(@Param("userID")Long userID);
