...
public interface UsersInfoRepository extends JpaRepository<UsersInfo, Long> {
    @Query("SELECT new com.student.book_advisor.dto.UsersInfoDTO(ui.id, ui.name, ui.surname, ui.user.username, ui.email, ui.description) FROM UsersInfo ui WHERE ui.user.username = :username")
    public UsersInfoDTO findByUsername(@Param("username")String username);

    @Query("SELECT new com.student.book_advisor.dto.UsersInfoDTO(ui.id, ui.name, ui.surname, ui.user.username, ui.email, ui.description) FROM UsersInfo ui WHERE ui.id = :id")
    public UsersInfoDTO findUserById(@Param("id")Long id);

    @Query("SELECT new com.student.book_advisor.dto.UtenteCardDTO(ui.id, ui.user.username, ui.name, ui.surname) FROM UsersInfo ui")
    ...
    @Query("SELECT count(ui) FROM UsersInfo ui WHERE ui.user.username = :username")
